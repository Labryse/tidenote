...
/** 
 * Soft-delete a canvas file by marking its chunk docs with a deletedAt timestamp.
 * This prevents immediate data loss in case of a race condition.
 */
export async function softDeleteCanvasFile(noteId: string, fileId: string, chunks: number): Promise<void> {
  const filesCol = collection(db, "notes", noteId, "files");
  const batch = writeBatch(db);
  const now = Date.now();
  for (let i = 0; i < Math.max(1, chunks); i++) {
    batch.update(doc(filesCol, `${fileId}__${i}`), { deletedAt: now });
  }
  await batch.commit();
}
 ...
 * a data URL keyed by fileId. Cleans up soft-deleted files older than 7 days.
 */
export async function loadCanvasFiles(
  ...
  const filesCol = collection(db, "notes", noteId, "files");
  const snap = await getDocs(filesCol);
  const grouped: Record<string, { mimeType: string; parts: string[]; deletedAt?: number; chunks: number }> = {};
  ...
  snap.forEach((d) => {
    ...
    // Physical GC check: if older than 7 days, mark for hard delete.
    if (data.deletedAt && (now - data.deletedAt > SEVEN_DAYS_MS)) {
      docsToDelete.push(d.ref);
      return; // Skip loading this chunk
    }

    if (!grouped[fid]) grouped[fid] = { mimeType: "image/webp", parts: [], chunks: data.chunks || 1, deletedAt: data.deletedAt };
    if (data.mimeType) grouped[fid].mimeType = data.mimeType;
    if (typeof data.index === "number") grouped[fid].parts[data.index] = data.data || "";
  });

  // Hard delete expired chunks in batches
  if (docsToDelete.length > 0) {
    for (let i = 0; i < docsToDelete.length; i += 400) {
      const batch = writeBatch(db);
      for (const ref of docsToDelete.slice(i, i + 400)) batch.delete(ref);
      batch.commit().catch(e => console.error("Canvas GC hard delete failed:", e));
    }
  }

  const out: Record<string, { dataURL: string; mimeType: string }> = {};
  for (const [fid, v] of Object.entries(grouped)) {
    // Only assemble if we got some parts
