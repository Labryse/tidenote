...
export default function Canvas() {
  ...
  // Layer 1 GC Guard: track when the canvas initial data was set
  const initialLoadTimeRef = useRef<number>(0);
  // Layer 2 GC Guard: track previous number of images to detect suspicious drops
  const previousUsedFileIdsRef = useRef<Set<string>>(new Set());
  ...
  const saveCanvasToFirestore = async (noteId: string, elements: any[], appState: any, files: any) => {
    ...
    try {
      ...
      // --- GC Guard: Layer 1 (Initial Load) ---
      const timeSinceLoad = Date.now() - initialLoadTimeRef.current;
      const isInitialLoadPhase = timeSinceLoad < 5000;

      // --- GC Guard: Layer 2 (Suspicious Drop) ---
      const previousCount = previousUsedFileIdsRef.current.size;
      const currentCount = usedFileIds.size;
      // If we had images, but now have none, and total elements are very few, it's highly suspicious.
      const isSuspiciousDrop = previousCount > 0 && currentCount === 0 && cleanElements.length < 5;

      let skipGC = false;
      if (isInitialLoadPhase) {
        console.log("Canvas GC skipped: Initial load phase (protecting against race conditions).");
        skipGC = true;
      } else if (isSuspiciousDrop) {
        console.warn("Canvas GC skipped: Suspicious drop in image elements detected.");
        skipGC = true;
      }

      previousUsedFileIdsRef.current = usedFileIds;

      if (!skipGC) {
        // Soft-delete subcollection docs for images that are no longer on the canvas.
        for (const fileId of Object.keys(persistedFilesRef.current)) {
          if (!usedFileIds.has(fileId)) {
            ...
            softDeleteCanvasFile(noteId, fileId, stale.chunks).catch((e) =>
              console.error("Canvas file soft delete failed:", e)
            );
          }
        }
      }
  ...
  useEffect(() => {
    ...
    previousUsedFileIdsRef.current = new Set();
