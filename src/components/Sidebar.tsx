...
import { FilePlus, PenSquare, LayoutTemplate, Users, CalendarDays, Lightbulb, Kanban, ChevronLeft, ChevronRight, ChevronDown, LogOut, MoreVertical, FolderPlus, Folder as FolderIcon, FolderOpen, Archive, Palette, Bug, Laptop, Sparkles, Filter, Star, Clock, Tag, Zap, Bookmark, X } from "lucide-react";
...
export default function Sidebar() {
  ...
  useHardwareBack(isMobileSidebarOpen, () => setIsMobileSidebarOpen(false));
  ...
  return (
    <aside 
      className={`sidebar ${effectiveIsCollapsed ? "collapsed" : ""} ${isMobileSidebarOpen ? "open" : ""}`}
    ...
    >
      ...
      <div className="sidebar-header">
        <div className="sidebar-brand-row" style={{ display: "flex", alignItems: "center", justifyContent: (effectiveIsCollapsed && !isMobile) ? "center" : "space-between", width: "100%", position: "relative", minHeight: "32px" }}>
          {(!effectiveIsCollapsed || isMobile) ? (
            <h1 className="app-title" onClick={() => { navigate("/"); setActiveNoteId(null); if (isMobile) setIsMobileSidebarOpen(false); }} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", margin: 0 }}>
              <img 
                src={logoSrc} 
        ...
        <div className={`sidebar-top-actions ${effectiveIsCollapsed ? "collapsed-flex" : ""}`}>
          {/* Dropdown triggers */}
          <div className="new-note-container" ref={dropdownContainerRef} style={{ marginBottom: effectiveIsCollapsed ? 0 : undefined }}>
          <button 
            className="new-note-btn" 
            onClick={() => setIsNewNoteDropdownOpen(!isNewNoteDropdownOpen)} 
            disabled={!user}
            title={effectiveIsCollapsed ? t("sidebar.newNote") : undefined}
          >
            <svg
              width="16"
              height="16"
            ...
            {!effectiveIsCollapsed && t("sidebar.newNote")}
          </button>
        ...
        {user && (
          <div className="journal-sidebar-section" style={{ width: "100%", display: "flex", flexDirection: "column", gap: "4px", marginBottom: effectiveIsCollapsed ? "0" : "8px" }}>
            <button 
        ...
        {/* Archive Button for Collapsed Mode (moved to top actions) */}
        {user && effectiveIsCollapsed && archivedCount > 0 && (
          <button
            type="button"
        ...
        {user && !effectiveIsCollapsed && allUniqueTags.length > 0 && !showArchived && (
      ...
      {user && (
        <div className="sidebar-footer">
          {!effectiveIsCollapsed && archivedCount > 0 && (
            <div className="sidebar-footer-row" style={{ borderBottom: "1px solid var(--color-border)", paddingBottom: "6px", marginBottom: "4px", justifyContent: "flex-start", display: "flex" }}>
              <button
                type="button"
                className={`sidebar-archive-toggle-btn ${showArchived ? "active" : ""}`}
                onClick={() => {
              ...
              >
                <Archive size={16} />
                <span> {t("sidebar.archiveTitle")} ({archivedCount})</span>
              </button>
            </div>
          )}


          {isWebOnly() && !isCapacitor() && !isMobile && !isDismissed && (
            <div className={`sidebar-download-banner ${effectiveIsCollapsed ? "collapsed" : ""}`}>
              {effectiveIsCollapsed ? (
                <a
                  href={downloadUrl}
                  download
          ...
          <div className="sidebar-footer-row" style={{ display: "flex", flexDirection: effectiveIsCollapsed ? "column" : "row", alignItems: "center", gap: effectiveIsCollapsed ? "12px" : "8px", width: "100%" }}>
            <button
              type="button"
